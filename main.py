...
Scrapes 8,000+ fintech app reviews weekly across four apps (Groww, INDmoney, Zerodha Kite, Stable Money), clusters them with UMAP + HDBSCAN, and runs a 5-stage anti-hallucination layer (fuzzy matching) that proves every quote exists verbatim in the source before it ships. Delivers a prioritised weekly fix-list for PMs. Runs autonomously via GitHub Actions every Monday, costs $0 (free-tier APIs), and uses MCP to write reports to Google Docs + draft Gmail summaries. Stack: Python, SQLite, HDBSCAN, UMAP, RapidFuzz, Groq (Llama 3), Gemini embeddings, React, Vite, Tailwind, Recharts, GitHub Actions. Built during the Next Leap fellowship.

## PULSE — Compare view (cross-app benchmarking, added later)
A dedicated "Compare" tab that ranks all four tracked apps side by side for any shared week: rating, 1-star rate, App Store/Play Store split, top complaint with an LLM-written business-impact line, a persistence badge (e.g. "6wks unresolved" vs "New this week"), and a per-app data-quality strip (evidence score, % of reviews filtered as noise). Below that: a rating + review-volume trend chart, a "Shared pain points" matrix that surfaces which problem categories (support, app stability, fees, KYC, fulfillment) are hitting 2+ apps at once — a real cross-company pattern, not one app's noise — and a team-pressure rollup showing which functions (Customer Support, Android Eng, etc.) are under the most complaint load industry-wide. Also flags partial data (e.g. a week missing an expected source) rather than silently comparing incomplete data as if it were complete.
